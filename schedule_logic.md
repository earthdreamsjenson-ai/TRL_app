...
### 優先①：遠方制約に基づく優先割り当て
* **仕様**: 以下の優先順位に従ってスロット（グラウンド）および対戦カードの組み合わせを試行します。
  1. **通常グラウンドに対して、遠方NGのチームを含む試合を優先して割り付け**
  2. **①の割り当て後、通常グラウンドが残っていたら、遠方OKのチーム同士の試合を割り付け**
  3. **遠方OKのチーム同士の試合を、遠方グラウンドに割り付け**
* **背景**: 遠方NGチームは通常グラウンドにしか配置できないため、通常グラウンドを先に遠方NGチームの試合で優先的に埋める必要があります。そうしないと通常グラウンドが遠方OKチーム同士の試合で埋まってしまい、遠方NGチームの試合が組めなくなるためです。

### 優先②：残り試合数の多いチームの優先割り当て
* **仕様**: 未決定の対戦プール (`match_pool`) の中で、**残り試合数（ホームアウェイ合計）が多いチームの対戦を優先的**に候補として処理します（同じ優先度グループ内での試行順序に適用されます）。
* **ソート順の基準**:
...
```mermaid
graph TD
    A[スタート: make_monthly_schedule] --> B[1. スロットリストを通常グラウンド優先でソート]
    B --> C[2. 対戦リストを残試合数が多いチーム優先でソート]
